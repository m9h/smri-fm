#!/usr/bin/env bash
# Finetune the SIAM FM on FOMO26 Task 3 (REGR002 brain-age regression), inside an
# NVIDIA PyTorch (NGC) container with --gpus all. Same env/dep strategy as the
# CLS002 driver (PYTHONPATH for asparagus; constraint-pinned deps so NGC torch is
# not downgraded). REGR002 is 1-channel / 1-output, so SIAM's 1ch stem loads
# directly (no repeat) and the head is a single age regressor.
#
# Expected mounts (set by the docker wrapper):
#   /workspace/smri-fm   <- repo root
#   /fomo26              <- /data/datasets/fomo26
#   /siam_params         <- ~/siam_params
#
...
CKPT=/fomo26/siam_regr002_asparagus.ckpt
...
if [ "$DEBUG" = "1" ]; then
  ...
  RUNDIR=$ASPARAGUS_MODELS/regr002_siam_debug
  ...
  RUNDIR=$ASPARAGUS_MODELS/regr002_siam
