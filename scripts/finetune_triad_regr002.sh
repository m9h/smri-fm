#!/usr/bin/env bash
# Finetune the Triad (wangshansong1 SwinViT-V2, MAE-pretrained on ~131k vols) FM
# on FOMO26 Task 3 (REGR002 brain-age regression), inside an NVIDIA PyTorch
# (NGC) container with --gpus all. Mirrors finetune_fomo60k_regr002.sh: same
# Swin family (depths 2/2/2/2 vs FOMO60K 2/2/6/2), so the same 128^3 crop /
# batch 2 / 1ch stem / single age head apply. PYTHONPATH for asparagus,
# constraint-pinned deps so NGC torch is not downgraded.
#
# Expected mounts (set by the docker wrapper):
#   /workspace/smri-fm   <- repo root
#   /fomo26              <- /data/datasets/fomo26   (also holds the Triad ckpt)
#
...
TRIAD_CKPT=${TRIAD_CKPT:-/fomo26/weights/triad/Triad-SwinB-MAE.pth}
...
echo ">>> converting Triad checkpoint -> asparagus format"
CKPT=/fomo26/triad_regr002_asparagus.ckpt
python -c "from asparagus_bridge.models_smri_triad import convert_triad_checkpoint; convert_triad_checkpoint('$TRIAD_CKPT', '$CKPT'); print('wrote', '$CKPT')"

DEBUG=${DEBUG:-1}
if [ "$DEBUG" = "1" ]; then
  echo ">>> DEBUG smoke finetune (5 epochs, 96^3 crop, limited batches)"
  RUNDIR=$ASPARAGUS_MODELS/regr002_triad_debug
  ...
  RUNDIR=$ASPARAGUS_MODELS/regr002_triad
