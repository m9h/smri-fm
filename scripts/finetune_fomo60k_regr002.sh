#!/usr/bin/env bash
# Finetune the FOMO60K (pkoutsouvelis SwinViT-V2 MAE) FM on FOMO26 Task 3
# (REGR002 brain-age regression), inside an NVIDIA PyTorch (NGC) container with
# --gpus all. Mirrors finetune_siam_regr002.sh: PYTHONPATH for asparagus,
# constraint-pinned deps so NGC torch is not downgraded. REGR002 is 1-channel /
# 1-output, so the 1ch SwinViT stem loads directly and the head is a single age
# regressor.
#
# Expected mounts (set by the docker wrapper):
#   /workspace/smri-fm   <- repo root
#   /fomo26              <- /data/datasets/fomo26   (also holds the FOMO60K ckpt)
#
...
FOMO60K_CKPT=${FOMO60K_CKPT:-/fomo26/weights/fomo60k_pkoutsouvelis/combined_regular-step=200000.ckpt}
...
echo ">>> converting FOMO60K checkpoint -> asparagus format"
CKPT=/fomo26/fomo60k_regr002_asparagus.ckpt
python -c "from asparagus_bridge.models_smri_fomo60k import convert_fomo60k_checkpoint; convert_fomo60k_checkpoint('$FOMO60K_CKPT', '$CKPT'); print('wrote', '$CKPT')"

DEBUG=${DEBUG:-1}
if [ "$DEBUG" = "1" ]; then
  echo ">>> DEBUG smoke finetune (5 epochs, 96^3 crop, limited batches)"
  RUNDIR=$ASPARAGUS_MODELS/regr002_fomo60k_debug
  ...
  RUNDIR=$ASPARAGUS_MODELS/regr002_fomo60k
