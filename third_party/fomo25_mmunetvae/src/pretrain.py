...
def main():
    ...
    # We get the data per-subject
    parent_dir = os.path.dirname(path_config.train_data_dir)
    path_dict_structure = os.path.join(parent_dir, 'subject_level_dict.pkl')
    splits_config, data_dict = get_pretrain_split_config(
        method="simple_train_val_split",
        idx=0,
        split_ratio=0.1, # We use 10% of data for validation split / per subject
        path_config=path_config,
        per_subject=True,
        path_dict_structure=path_dict_structure
    )
