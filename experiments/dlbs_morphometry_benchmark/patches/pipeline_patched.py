...
def run_synthseg(
    ...
    # PATCH 2026-04-29: container's mri_synthseg is at
    # /usr/lib/freesurfer-python/bin/mri_synthseg (executable script with
    # its own shebang); /opt/venv/bin/python no longer exists. Call the
    # script directly instead of `python <script>`.
    import shutil as _sh
    synthseg_bin = _sh.which("mri_synthseg") or "/usr/lib/freesurfer-python/bin/mri_synthseg"
    cmd = [
        synthseg_bin,
        "--i", str(input_txt),
        "--o", str(output_txt),
    ...
    if result.returncode != 0:
        # PATCH: dump stdout+stderr so we can see SynthSeg's actual complaint
        log.error("mri_synthseg STDOUT (last 4kB):\n%s", (result.stdout or "")[-4000:])
        log.error("mri_synthseg STDERR (last 4kB):\n%s", (result.stderr or "")[-4000:])
